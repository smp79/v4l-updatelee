...
//	pr_info("%s: MATYPE: %02x\n", __func__, MAKEWORD16(STV091X_READ_REG(state, MATSTR1), STV091X_READ_REG(state, MATSTR0)));
//	pr_info("%s: UPLSTR: %02x\n", __func__, MAKEWORD16(STV091X_READ_REG(state, UPLSTR1), STV091X_READ_REG(state, UPLSTR0)));
//	pr_info("%s: DFLSTR: %02x\n", __func__, MAKEWORD16(STV091X_READ_REG(state, DFLSTR1), STV091X_READ_REG(state, DFLSTR0)));
//	pr_info("%s: SYNCSTR: %02x\n", __func__, STV091X_READ_REG(state, SYNCSTR));
//	pr_info("%s: SYNCDSTR: %02x\n", __func__, MAKEWORD16(STV091X_READ_REG(state, SYNCDSTR1), STV091X_READ_REG(state, SYNCDSTR0)));
...
//	pr_info("%s: demod: %d\n", __func__, state->nr);
	...
	if (id != 0x51)
		return -EINVAL;
	pr_info("stv091x: found STV091X id=0x%02x\n", id);
...
static int stv091x_set_property(struct dvb_frontend *fe,
...
static int stv091x_get_property(struct dvb_frontend *fe, struct dtv_property *tvp)
{
	return 0;
}
...
static int stv091x_set_dfmt(struct dvb_frontend *fe,  enum fe_data_format dfmt)
{
	struct stv091x_state *state = fe->demodulator_priv;
	...
	case FE_DFMT_TS_PACKET:
		pr_info("%s: FE_DFMT_TS_PACKET\n", __func__);
		STV091X_WRITE_FIELD(state, FORCE_CONTINUOUS, 0);
		STV091X_WRITE_FIELD(state, FRAME_MODE, 0);
		break;
	case FE_DFMT_BB_FRAME:
		pr_info("%s: FE_DFMT_BB_FRAME\n", __func__);
		STV091X_WRITE_FIELD(state, FORCE_CONTINUOUS, 1);
		STV091X_WRITE_FIELD(state, FRAME_MODE, 2);
		break;
	default:
		pr_info("%s: Invalid data format\n", __func__);
		return -1;
	}
	STV091X_WRITE_FIELD(state, RST_HWARE, 1);
	STV091X_WRITE_FIELD(state, RST_HWARE, 0);
...
static struct dvb_frontend_ops stv091x_ops = {
	...
	.set_property			= stv091x_set_property,
	.get_property			= stv091x_get_property,
