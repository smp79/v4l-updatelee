...
static struct stv090x_long_frame_crloop	stv090x_s2_apsk_crl_cut30[] = {
	/* MODCOD  2MPon 2MPoff 5MPon 5MPoff 10MPon 10MPoff 20MPon 20MPoff 30MPon 30MPoff */
	{ STV090x_16APSK_23,  0x0a, 0x0a, 0x0a, 0x0a, 0x1a, 0x0a, 0x39, 0x0a, 0x29, 0x0a },
	{ STV090x_16APSK_34,  0x0a, 0x0a, 0x0a, 0x0a, 0x0b, 0x0a, 0x2a, 0x0a, 0x1a, 0x0a },
	{ STV090x_16APSK_45,  0x0a, 0x0a, 0x0a, 0x0a, 0x1b, 0x0a, 0x3a, 0x0a, 0x2a, 0x0a },
	{ STV090x_16APSK_56,  0x0a, 0x0a, 0x0a, 0x0a, 0x1b, 0x0a, 0x3a, 0x0a, 0x2a, 0x0a },
	{ STV090x_16APSK_89,  0x0a, 0x0a, 0x0a, 0x0a, 0x2b, 0x0a, 0x0b, 0x0a, 0x3a, 0x0a },
	{ STV090x_16APSK_910, 0x0a, 0x0a, 0x0a, 0x0a, 0x2b, 0x0a, 0x0b, 0x0a, 0x3a, 0x0a },
	{ STV090x_32APSK_34,  0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09 },
	{ STV090x_32APSK_45,  0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09 },
	{ STV090x_32APSK_56,  0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09 },
	{ STV090x_32APSK_89,  0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09 },
	{ STV090x_32APSK_910, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09 }
};
...
static int stv090x_read_reg(struct stv090x_state *state, unsigned int reg)
{
	...
	if (ret != 2) {
		if (ret != -ERESTARTSYS)
			dprintk("Read error, Reg=[0x%02x], Status=%d", reg, ret);

		return ret < 0 ? ret : -EREMOTEIO;
	}
	if (unlikely(*state->verbose >= FE_DEBUGREG))
		dprintk("Reg=[0x%02x], data=%02x", reg, buf);
...
static int stv090x_write_regs(struct stv090x_state *state, unsigned int reg, u8 *data, u32 count)
{
	...
	if (ret != 1) {
		if (ret != -ERESTARTSYS)
			dprintk("Reg=[0x%04x], Data=[0x%02x ...], Count=%u, Status=%d",
...
static u32 stv090x_srate_srch_coarse(struct stv090x_state *state)
{
	...
	while ((!tmg_lock) && (cur_step < steps)) {
		...
		if ((tmg_cpt >= 5) && (agc2 < agc2th) &&
			...
			if (reg)
				dprintk("Tuner phase locked");
			else
				dprintk("Tuner unlocked");
...
static int stv090x_get_dmdlock(struct stv090x_state *state, s32 timeout)
{
	...
	while ((timer < timeout) && (!lock)) {
		...
		if (!lock)
			...
			dprintk("Demodulator acquired LOCK");

		timer += 10;
	}
	dprintk("lock: %s", lock ? "true" : "false");
	return lock;
}
...
static enum stv090x_signal_state stv090x_get_sig_params(struct stv090x_state *state)
{
	...
	state->matype = (STV090x_READ_DEMOD(state, MATSTR1) << 8) + STV090x_READ_DEMOD(state, MATSTR0);
	...
	props->frame_len	= state->frame_len;
...
static enum stv090x_signal_state stv090x_algo(struct stv090x_state *state)
{
	...
	if ((agc1_power == 0) && (power_iq < STV090x_IQPOWER_THRESHOLD)) {
		dprintk("No Signal: POWER_IQ=0x%02x", power_iq);
		lock = 0;
...
static enum dvbfe_search stv090x_search(struct dvb_frontend *fe)
{
	...
	if (props->enable_modcod == 0x0fffffff) {
		...
		dprintk("mod %08x state->algo = STV090x_BLIND_SEARCH", props->enable_modcod);
		state->search_range = 0;
	} else {
		state->algo = STV090x_COLD_SEARCH;
		dprintk("mod %08x state->algo = STV090x_COLD_SEARCH", props->enable_modcod);
		if (props->symbol_rate > 10000000) {
	...
	if (stv090x_algo(state) == STV090x_RANGEOK) {
		...
		dprintk("frequency       = %d", props->frequency);
		dprintk("symbol_rate     = %d", props->symbol_rate);
		dprintk("fec_inner       = %d, %d", props->fec_inner, state->modcod);
		dprintk("pilot           = %d", props->pilot);
		dprintk("rolloff         = %d", props->rolloff);
		...
		if (frontend_ops->set_frame_ops) {
			...
			dprintk("frame_ops.frame_size = %d", frame_ops.frame_size);
		}

		return DVBFE_ALGO_SEARCH_SUCCESS;
	} else {
		dprintk("Search failed!");
...
static s32 stv090x_table_lookup(const struct stv090x_tab *tab, u16 max, u16 val)
{
	u16 min = 0;
	u16 med;
	s32 res = 0;
...
static s32 stv090x_read_dbm(struct dvb_frontend *fe)
{
	struct stv090x_state *state = fe->demodulator_priv;
	u16 reg, agc_0, agc_1, agc;
	s32 str;
...
{
	struct stv090x_state *state = fe->demodulator_priv;
	u16 reg, agc_0, agc_1, min, max, dif;
	u32 agc, tmp;
	...
	min = stv090x_rf_tab[ARRAY_SIZE(stv090x_rf_tab) - 1].read;
	max = stv090x_rf_tab[0].read;
	dif = max - min;
	tmp = ((agc - min) << 16) / dif;
...
static u64 stv090x_read_ucb(struct dvb_frontend *fe)
{
	struct stv090x_state *state = fe->demodulator_priv;
	u8 err_val1, err_val0;
	u32 reg, header_err_val = 0;
...
static int stv090x_get_stats(struct dvb_frontend *fe, enum fe_status stat)
{
	struct stv090x_state *state = fe->demodulator_priv;
	...
	c->cnr.stat[1].uvalue = div_u64(c->cnr.stat[0].svalue, 100) * 328;
...
static int stv090x_read_status(struct dvb_frontend *fe, enum fe_status *status)
{
	...
	*status = 0;
	...
	case 1: /* first PLH detected */
	default:
		dprintk("Status: Unlocked (Searching ..)");
		*status = 0;
		break;
	case 2: /* DVB-S2 mode */
...
static int stv090x_set_frontend(struct dvb_frontend *fe)
{
	...
	struct dtv_frontend_properties *props = &fe->dtv_property_cache;
	if (props->enable_modcod == 0x0fffffff) {
		dprintk("state->algo = STV090x_BLIND_SEARCH");
		state->algo = STV090x_BLIND_SEARCH;
	} else {
		dprintk("state->algo = STV090x_COLD_SEARCH");
		state->algo = STV090x_COLD_SEARCH;
	}
...
static int stv0900_set_tspath(struct stv090x_state *state)
{
	...
	if (ts_nosync)
	{
		dprintk("TS FIFO Minimum Latence mode\n");
...
static int stv090x_setup(struct dvb_frontend *fe)
{
	...
	/* write initval */
	dprintk("Setting up initial values");
	for (i = 0; i < t1_size; i++) {
	...
	if (state->internal->dev_ver >= 0x20) {
		...
		/* write cut20_val*/
		dprintk("Setting up Cut 2.0 initial values");
		for (i = 0; i < t2_size; i++) {
		...
		dprintk("ERROR: Unsupported Cut: 0x%02x!",
			state->internal->dev_ver);

		goto err;
	} else if (state->internal->dev_ver > 0x30) {
		/* we shouldn't bail out from here */
		dprintk("INFO: Cut: 0x%02x probably incomplete support!",
			state->internal->dev_ver);
	}
...
static int stv090x_read_reg2(struct stv090x_state *state, unsigned int reg, u8* out, u8 len)
{
	const struct stv090x_config *config = state->config;
	int ret;
	...
	struct i2c_msg msg[] = {
		{ .addr	= config->address, .flags	= 0, 		.buf = b0,	.len = 2 },
		{ .addr	= config->address, .flags	= I2C_M_RD,	.buf = out,	.len = len }
	};
...
static int stv090x_get_consellation_samples(struct dvb_frontend *fe, struct dvb_fe_constellation_samples *s)
{
	struct stv090x_state *state = fe->demodulator_priv;
	u32 x;
	u8 buf[2];
...
static int stv090x_get_spectrum_scan(struct dvb_frontend *fe, struct dvb_fe_spectrum_scan *s)
{
	...
	reg = stv090x_read_reg(state, STV090x_P1_TSCFGH);
	STV090x_SETFIELD_Px(reg, RST_HWARE_FIELD, 0x01);
	stv090x_write_reg(state, STV090x_P1_TSCFGH, reg);

	reg = STV090x_READ_DEMOD(state, DMDISTATE);
	STV090x_SETFIELD_Px(reg, I2C_DEMOD_MODE_FIELD, 0x1f);
	STV090x_WRITE_DEMOD(state, DMDISTATE, reg);

	stv090x_write_reg(state, STV090x_P1_AGC2REF, 0x38);
				    ...
				    enum stv090x_demodulator demod)
{
	...
	if ((temp_int != NULL) && (state->demod_mode == STV090x_DUAL)) {
		...
		state->internal->num_used++;
		dprintk("Found Internal Structure!");
	} else {
		...
		dprintk("Create New Internal Structure!");

		mutex_init(&state->internal->demod_lock);
		mutex_init(&state->internal->tuner_lock);

		if (stv090x_setup(&state->frontend) < 0) {
			dprintk("Error setting up device");
			goto err_remove;
		}
	}
	...
	dprintk("Attaching %s demodulator(%d) Cut=0x%02x",
	       state->device == STV0900 ? "STV0900" : "STV0903",
	       demod,
	       state->internal->dev_ver);
